...
| 2   | 快捷键被占用 / 被系统吞掉         | 中    | 用户困惑     | 注册失败即时提示；引导更换                |
...
工程实现过程中的两个关键经验（可作后续参考）：

1. 窗口过程在 `WM_NCCREATE` / `WM_CREATE` 阶段必须使用回调传入的 `hwnd` 参数调用 `DefWindowProc`（此时成员 `hwnd_` 尚未赋值，使用成员会导致窗口创建失败）；
2. 调试辅助：设置环境变量 `EASYMUTE_DEBUG=1` 输出内部调试日志（`%TEMP%\easymute_debug.log`）；`easymute_soundstub.exe` 可制造真实音频会话用于验证。

备注：开发机上默认组合 `Ctrl+Alt+M` 已被其他软件占用（属既成事实，程序会按设计提示更换）；实测使用 `Ctrl+Alt+F9` 完成全部验证。
