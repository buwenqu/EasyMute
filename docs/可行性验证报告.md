...
| 2   | 快捷键被占用 / 被系统吞掉         | 中    | 用户困惑     | 设置不拦截、不检测；注册失败静默保留旧键，界面不提示（2026-09-11 / 09-12 变更） |
...
备注：开发机上默认组合 `Ctrl+Alt+M` 已被其他软件占用（属既成事实）。按 2026-09-11 / 09-12 需求变更，程序不再提示 / 拦截"被占用"，也不做占用检测与注册结果标注（完全静默）；实测使用无冲突的 `Ctrl+Alt+F9` 完成全部验证。
